...
export function matchCaseStudies(prospect, count = 6) {
  const pinned = (prospect.pinnedCaseStudies || [])
    .map(id => CASE_STUDIES.find(cs => cs.id === id))
    .filter(Boolean);
  const pinnedIds = new Set(pinned.map(cs => cs.id));
  ...
  const scored = CASE_STUDIES
    .filter(cs => !pinnedIds.has(cs.id))
    .map((cs) => {
  ...
  return [...pinned, ...scored.sort((a, b) => b.score - a.score)].slice(0, count);
}
...
export function getHighlightedServices(priorities = []) {
  const map = {
    "paid-search":   ["Paid Search — Google & Microsoft"],
    "paid-social":   ["Paid Social — Meta & LinkedIn"],
    "ecommerce":     ["Ecommerce & Shopping"],
    "lead-gen":      ["Lead Generation & B2B"],
    "youtube":       ["Programmatic & YouTube"],
    "cro":           ["Conversion Rate Optimization"],
    "creative":      ["AI-Native Video Production", "Performance Static & Display"],
    "ai-transform":  ["AI Transformation & Implementation"],
    "seo":           ["SEO & Answer Engine Optimization"],
    "training":      ["Training & Consulting"],
  };
  return priorities.flatMap((p) => map[p] || []);
}
